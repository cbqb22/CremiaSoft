...
namespace CremiaView.UI.Printing
{
    class PrintCenter
    {
        ...
        public void CreateImageFromXPS()
        {
            ...
            _FilePath = System.IO.Path.Combine(dir, DateTime.Now.ToString("yyyyMMdd.HHmmss") + ".png");

            using (Stream stream = System.IO.File.Create(_FilePath))
            {
                png.Save(stream);
            }
        ...
        {
            //画像を読み込む
            System.Drawing.Image img = System.Drawing.Image.FromFile(_FilePath);
            //画像を描画する
